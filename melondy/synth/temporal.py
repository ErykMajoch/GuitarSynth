...
class Time:
    ...
    def __add__(self, seconds: Numeric | Self) -> Self:
        match seconds:
            ...
            case _:
                raise TypeError(f"Can't add {type(seconds.__name__)} to Time instance")
    ...
    def __mul__(self, seconds: Numeric) -> Self:
        match seconds:
            ...
            case _:
                raise TypeError(
                    f"Can't multiply {type(seconds.__name__)} with Time instance"
                )
...
class MeasuredTimeline(Timeline):
    ...
    def __next__(self) -> Self:
        if self.measure.seconds <= 0:
            raise ValueError(f"Measure duration must be a positive value")
